...
<!--      </el-table-column>-->
      <el-table-column label="操作" fixed="right" min-width="150" align="center">
        <template slot-scope="scope">
          <el-button v-if=" (scope.row.orderStatus === 1 ||scope.row.orderStatus === 4)&& (RoleSrting.indexOf('5') !== -1||RoleSrting.indexOf('6') !== -1)" type="text" icon="el-icon-edit" style="cursor: pointer" @click="handleDispatch(scope.row)">派单</el-button>
          <el-button v-if=" scope.row.orderStatus === 2 && (RoleSrting.indexOf('5') !== -1||RoleSrting.indexOf('6') !== -1)" type="text" icon="el-icon-edit" style="cursor: pointer" @click="handleRecallDispatch(scope.row)">退单</el-button>
          <el-button v-if=" scope.row.orderStatus === 5 && (RoleSrting.indexOf('5') !== -1||RoleSrting.indexOf('6') !== -1)" type="text" icon="el-icon-edit" style="cursor: pointer" @click="handleAduitDispatch(scope.row)">审核</el-button>
          <el-button v-if=" scope.row.orderStatus === 0 && (RoleSrting.indexOf('5') !== -1||RoleSrting.indexOf('6') !== -1)" type="text" icon="el-icon-edit" @click="handleEdit(scope.row)">编辑</el-button>
          <!--           <el-button  type="text" icon="el-icon-edit" style="cursor: pointer" v-if=" scope.row.orderStatus === 7  && RoleSrting.indexOf('5') !== -1" @click="handleAduitDispatch(scope.row)" >复勘</el-button>-->
          <el-button v-if=" (scope.row.orderStatus === 0 ||scope.row.orderStatus === 7) && (RoleSrting.indexOf('5') !== -1||RoleSrting.indexOf('6') !== -1)" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-collapse v-model="activeNames">
        <el-collapse-item title="基本信息" name="1">
          ...
          >
            <el-row>
              <el-col :span="8">
                <el-form-item label="工单编号" prop="biTian1" :label-width="formLabelWidth">
                  <el-input v-model="dataForm.orderId" :disabled="true" placeholder="请输入工具编号" />
                </el-form-item>
              </el-col>
              <el-col :span="8">
            ...
            <el-row>
              <el-col :span="22">
                <el-form-item label="作业内容" prop="workContent" :label-width="formLabelWidth">
                  <el-input v-model="dataForm.workContent" maxlength="256" :disabled="true" show-word-limit rows="3" type="textarea" placeholder="请输入作业内容" />
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="22">
                <el-form-item label="作业条件" prop="workContent" :label-width="formLabelWidth">
                  <el-input v-model="dataForm.workContent" maxlength="256" :disabled="true" show-word-limit rows="3" type="textarea" placeholder="请输入作业内容" />
                </el-form-item>
              </el-col>
            </el-row>
              ...
              <el-col :span="22">
                <el-form-item label="现场图片" prop="livePhoto" :label-width="formLabelWidth">
                  <el-upload
                    ...
                    :before-upload="beforeAvatarUpload"
                  >
                    <img v-if="dataForm.livePhoto" :src="dataForm.livePhoto" style="width: 150px;height: 120px" class="avatar">
                    <i v-else class="el-icon-plus  avatar-uploader-icon" />
                  </el-upload>
                </el-form-item>
              </el-col>
        ...
        <el-collapse-item title="派单信息" name="2">
          ...
          >
            <el-row>
              <el-col :span="8">
                <el-form-item label="施工单位" prop="jiedanUnit" :label-width="formLabelWidth">
              ...
              <el-col :span="8">
                <el-form-item label="施工班组" prop="jiedanTeam" :label-width="formLabelWidth">
              ...
              <el-col :span="8">
                <el-form-item label="施工负责人" prop="manage" :label-width="formLabelWidth">
            ...
            <el-row>
              <el-col :span="22">
                <el-form-item label="施工备注" prop="dispatchNote" :label-width="formLabelWidth">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button :size="size" @click="recallDialogVisible = false">取消</el-button>
        <el-button :size="size" type="primary" :loading="editLoading" @click="submitEditForm">确定</el-button>
      </span>
    </el-dialog>
    <!-- 审核界面 -->
    ...
    >
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item title="接单信息" name="2">
          <el-form
            ref="didpatchDataForm"
            :model="didpatchDataForm"
            :rules="dataRule"
            ...
            <el-row>
              <el-col :span="22">
                <el-form-item label="完工情况" prop="finishNote" :label-width="formLabelWidth">
                  <el-input v-model="handleDataForm.finishNote"  :disabled="true" maxlength="256" show-word-limit rows="5" type="textarea" placeholder="请输完工情况" />
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
        ...
        <el-collapse-item v-if="dataForm.orderStatus === 5||dataForm.orderStatus === 6||dataForm.orderStatus === 7 " title="审核信息" name="6">
          <el-form
            ref="dataForm1"
            :model="dataForm"
            :rules="dataRule"
          ...
          >
            ...
            <el-row>
              <el-col :span="22">
                <el-form-item label="审核原因" prop="auditReason" :label-width="formLabelWidth">
                  <el-input v-model="dataForm.auditReason" maxlength="256" show-word-limit rows="5" type="textarea" placeholder="请输入作业内容" />
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
...
<script>
import { deleteBatchCourse, isExistToolId } from '@/api/tool'
...
import { getWorkOrderInfoByPage, getOrderHandleInfoOne, auditWorkOrder } from '@/api/workOrder'
import { getUserInfoList } from '@/api/user'
import { addRequireDispatchInfo, getOrderAcceptDispatchInfo, getAcceptDispatchInfo, addOrderDispatchInfo, getRecallOrderDispatchInfo, recallOrderDispatch } from '@/api/dispatch'
...
export default {
  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        jiedanTeam: [{ required: true, message: '施工班组不能为空', trigger: 'blur' }],
        ...
        dispatchTime: [{ required: true, message: '必填项不能为空', trigger: 'blur' }],
        handleReason: [{ required: true, message: '必填项不能为空', trigger: 'blur' }],
        dispatchTime2: [{ required: true, message: '必填项不能为空', trigger: 'blur' }],
        dispatchTime3: [{ required: true, message: '必填项不能为空', trigger: 'blur' }],
        dispatchTime6: [{ required: true, message: '必填项不能为空', trigger: 'blur' }],
        dispatchTime4: [{ required: true, message: '必填项不能为空', trigger: 'blur' }],
        dispatchTime5: [{ required: true, message: '必填项不能为空', trigger: 'blur' }]
  ...
  methods: {
    ...
    getCourseList: function() {
      ...
      getWorkOrderInfoByPage(params).then(response => {
        this.loading = false
    ...
    // 获取需求信息并分页显示（待条件的查询）
    getWorkOrderInfoByPage: function() {
      this.loading = true
      const params = new URLSearchParams()
      params.append('current', this.currentPage)
      params.append('size', this.pageSize)
      ...
      if (this.isAudit) {
        this.$refs['dataForm1'].validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              this.editLoading = true
              auditWorkOrder(this.dataForm).then((res) => {
                if (res.data.code === 200) {
                  this.$message({ message: '操作成功', type: 'success' })
                } else {
                  this.$message({ message: res.data.msg, type: 'error' })
      ...
      if (this.isDispatch) {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              this.editLoading = true
              console.log('工作单派单提交的数据')
              console.log(this.dataForm)
              addOrderDispatchInfo(this.dataForm).then((res) => {
                if (res.data.code === 200) {
                  this.$message({ message: '操作成功', type: 'success' })
                } else {
                  this.$message({ message: res.data.msg, type: 'error' })
      ...
      if (this.isRecall) {
        this.$refs['didpatchDataForm'].validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              this.editLoading = true
              console.log('工作单退单提交的数据')
              console.log(this.didpatchDataForm)
