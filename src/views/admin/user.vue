          ...
          <el-form :model="dataForm" :rules="rules2" label-width="80px" size="small" label-position="right">
            ...
            <el-form-item label="部门" prop="Dept" :label-width="formLabelWidth">
              <popup-tree-input
                ...
                :node-key="dataForm.deptName"
                :current-change-handle="deptTreeCurrentChangeHandle"
              />
            </el-form-item>
            <el-form-item label="班组" prop="userRoles" label-width="120px">
              <el-select v-model="dataForm.teamName"  placeholder="请选择" @change="teamChange" style="width: 100%;">
                <el-option
...
export default {
  ...
  methods: {
    ...
    deptTreeCurrentChangeHandle(data) {
      ...
      const params = new URLSearchParams()
      params.append('teamUnitid', this.dataForm.deptId)
    ...
    submitForm: function() {
      ...
      if (!this.operation) {
        // 编辑用户
        // alert(this.dataForm.teamId)
        // this.dataForm.teamId = this.dataForm.teamName
        console.log('用户编辑页面提交前的班组为' + this.dataForm.teamId)
