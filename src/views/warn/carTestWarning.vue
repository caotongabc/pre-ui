  ...
  <div class="app-container" style="min-height: 100%">
    ...
    <el-table v-loading="loading" :data="tableData" border style="width: 100% " :default-sort="{ prop: 'startTime', order: 'ascending' }" @selection-change="handleSelectionChange">      <el-table-column type="selection" />
      ...
      <el-table-column label="状态" width="120" align="center">
        <template slot-scope="scope">
          <div>
            <el-tag v-if="scope.row.testState === 1" size="small">试验正常</el-tag>
            <el-tag v-else-if="scope.row.testState=== 2" size="small" type="info">试验不合格</el-tag>
          </div>
        </template>
      </el-table-column>
    ...
    <!-- 新增修改界面 -->
    <el-dialog
      :title="!dataForm.id ? '车辆试验处理' : '车辆试验处理'"
      width="70%"
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="所属仓库" prop="warehourseName" :label-width="formLabelWidth">
              <!--<el-input v-model="dataForm.courseDept" placeholder="请输培训部门" />-->
              <el-select v-model="dataForm.warehourseName"  :disabled="true"  @click.native="warehoueseClick" clearable placeholder="请选择" @change="wareCurrentChangeHandle2">
                <el-option
                  v-for="item in wareOptions"
                  :key="item.warehourseId"
                  :label="item.warehourseName"
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="车高(m)" prop="carHeight" :label-width="formLabelWidth">
              <el-input-number v-model="dataForm.carHeight" :disabled="true"  :min="0" :controls="false" :precision="2" placeholder="请输入车高"></el-input-number>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="22">
            <el-form-item label="备注" prop="carNote" :label-width="formLabelWidth">
              <el-input   maxlength="256"  :disabled="true"  show-word-limit rows=2 v-model="dataForm.carNote" type="textarea"  placeholder="请输入备注" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
        </el-collapse-item>
        <el-collapse-item title="车辆试验信息" name="2">
          <el-form
            ref="dataForm"
            :model="dataForm"
            :rules="dataRule"
...
import { fetchCarList, addCar, fetchCarWarnList, updateCar, deleteCar, deleteBatchCar, isExistCarNumber } from '@/api/car'
...
export default {
  ...
  methods: {
    parseTime,
    fetchCarWarnList2: function() {
      this.loading = true
      const params = new URLSearchParams()
      params.append('current', this.currentPage)
      params.append('size', this.pageSize)
      params.append('userUnit', this.unitID)
      fetchCarWarnList(params).then(response => {
        this.loading = false
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              addCarWarnTestInfo(this.dataForm).then((res) => {
