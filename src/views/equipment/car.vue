<template xmlns:font-color="http://www.w3.org/1999/xhtml">
  <div class="app-container" style="min-height: 100%">
    ...
    <div class="filter-container" style="margin: 10px 0 10px 0; min-height: 70%">
      ...
      <el-button class="filter-item " style="margin-top: 9px" size="small" type="primary" icon="el-icon-plus" @click="handleAdd">添加车辆</el-button>
    ...
    <el-table v-loading="loading" :data="tableData" border style="width: 100% " :default-sort="{ prop: 'startTime', order: 'ascending' }" @selection-change="handleSelectionChange">      <el-table-column type="selection" />
      ...
      <el-table-column label="车牌号" width="120" align="center" sortable prop="carNumber">
        <template slot-scope="scope">
          <span style="color: #2C7BFB ;cursor: pointer" @click="goDetail(scope.row.carNumber)">{{ scope.row.carNumber }}</span>
        </template>
      </el-table-column>
      <el-table-column label="车辆种类" width="130" align="center" sortable prop="bigType">
        <template slot-scope="scope">
      ...
      <el-table-column label="下次试验时间" width="120" align="center" sortable prop="nextTime">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.nextTestTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="下次保险时间" width="120" align="center" sortable prop="nextTime">
        <template slot-scope="scope">
      ...
      <el-table-column label="状态" width="120" align="center">
        <template slot-scope="scope">
          <div>
            <el-tag v-if="scope.row.testState=== 2" style="margin-bottom: 2px" size="small" type="info">试验不合格</el-tag>
          </div>
          <div>
            <el-tag v-if="scope.row.insuranceState=== 2" style="margin-bottom: 2px" size="small" type="info">保险不合格</el-tag>
          </div>
          <div>
            <el-tag v-if="scope.row.carState === 0" size="small">正常</el-tag>
          </div>
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        <el-row>
          <el-col :span="8">
            <el-form-item label="车牌号" prop="carNumber" :label-width="formLabelWidth">
              <el-input v-model="dataForm.carNumber" placeholder="请输入车牌号" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="所属仓库" prop="warehourseName" :label-width="formLabelWidth">
              <!--<el-input v-model="dataForm.courseDept" placeholder="请输培训部门" />-->
              <el-select v-model="dataForm.warehourseName" clearable placeholder="请选择" @click.native="warehoueseClick" @change="wareCurrentChangeHandle2">
                <el-option
                  v-for="item in wareOptions"
                  :key="item.warehourseId"
                  :label="item.warehourseName"
          ...
          <el-col :span="8">
            <el-form-item label="大类" prop="carType" :label-width="formLabelWidth">
              <!--<el-input v-model="dataForm.courseDept" placeholder="请输培训部门" />-->
              <el-select v-model="dataForm.carType" clearable placeholder="请选择">
                <el-option
                  v-for="item in carTypesOptions"
                  :key="item.code"
                  :label="item.value"
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="本次试验时间" prop="thisTestTime" :label-width="formLabelWidth">
              <el-date-picker
                v-model="dataForm.thisTestTime"
                type="datetime"
                format="yyyy-MM-dd HH:mm:ss"
                value-format="yyyy-MM-dd HH:mm:ss"
          ...
          <el-col :span="8">
            <el-form-item label="下次试验时间" prop="nextTestTime" :label-width="formLabelWidth">
              <el-date-picker
                v-model="dataForm.nextTestTime"
                :disabled="true"
                type="datetime"
                format="yyyy-MM-dd HH:mm:ss"
                value-format="yyyy-MM-dd HH:mm:ss"
          ...
          <el-col :span="8">
            <el-form-item label="本次保险时间" prop="thisInsuranceTime" :label-width="formLabelWidth">
              <el-date-picker
                v-model="dataForm.thisInsuranceTime"
                type="datetime"
                format="yyyy-MM-dd HH:mm:ss"
                value-format="yyyy-MM-dd HH:mm:ss"
                placeholder="选择结束日期时间"
                @change="thisInsuranceTimeChange"
              />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="8">
            <el-form-item label="下次保险时间" prop="nextInsuranceTime" :label-width="formLabelWidth">
              <el-date-picker
                v-model="dataForm.nextInsuranceTime"
                :disabled="true"
                type="datetime"
                format="yyyy-MM-dd HH:mm:ss"
                value-format="yyyy-MM-dd HH:mm:ss"
          ...
          <el-col :span="8">
            <el-form-item label="车长(m)" prop="carLength" :label-width="formLabelWidth">
              <el-input-number v-model="dataForm.carLength" :min="0" :controls="false" :precision="2" placeholder="请输入车长" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="车宽(m)" prop="carWidth" :label-width="formLabelWidth">
              <el-input-number v-model="dataForm.carWidth" :min="0" :controls="false" :precision="2" placeholder="请输入车宽" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="8">
            <el-form-item label="车高(m)" prop="carHeight" :label-width="formLabelWidth">
              <el-input-number v-model="dataForm.carHeight" :min="0" :controls="false" :precision="2" placeholder="请输入车高" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="车重(t)" prop="carWeight" :label-width="formLabelWidth">
              <el-input-number v-model="dataForm.carWeight" :min="0" :controls="false" :precision="2" placeholder="请输入车重" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="载重量(t)" prop="carZaizhong" :label-width="formLabelWidth">
              <el-input-number v-model="dataForm.carZaizhong" :min="0" :controls="false" :precision="2" placeholder="请输入载重量(t)" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="22">
            <el-form-item label="备注" prop="carNote" :label-width="formLabelWidth">
              <el-input v-model="dataForm.carNote" maxlength="256" show-word-limit rows="5" type="textarea" placeholder="请输入备注" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
import { fetchCarList, addCar, updateCar, deleteCar, deleteBatchCar, isExistCarNumber } from '@/api/car'
...
export default {
  ...
  methods: {
    ...
    getOneWarnStrategyInfo: function(message) {
      ...
      getOneWarnStrategyInfo(params).then(response => {
        ...
        if (response.data.data === null) {
          ...
          this.dataForm.nextTestTime = null
        } else {
          this.dataForm.expirationDate = response.data.data.expirationDate
          this.dataForm.nextTestTime = this.AddDayNumsDate(this.dataForm.thisTestTime, this.dataForm.expirationDate)
          console.log('获取到的预警策略数据')
          console.log(response.data.data)
          console.log(this.dataForm.expirationDate)
          console.log(this.dataForm.nextTestTime)
        }
      })
    },
    // 获取预警策略相关的信息(保险)
    ...
    getWareInfo2: function() {
      ...
      if (this.dataForm.teamId !== '' && this.dataForm.teamId !== undefined) {
        getWareInfo(params).then((res) => {
          this.wareOptions = res.data.data
          this.$forceUpdate()
          console.log('库房的信息')
    ...
    handleDelete: function(row) {
      if (row.carUseState === '使用中') {
        this.$message({
          showClose: true,
          message: '该车辆正在使用，不得删除!',
          type: 'warning'
        })
      } else {
        const that = this
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              updateCar(this.dataForm).then((res) => {
